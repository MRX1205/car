...
public class CheliangController {
    ...
    */
    ...
    public R update(@RequestBody CheliangEntity cheliang, HttpServletRequest request){
        ...
        if(cheliang.getId() == null){
            return R.error(511,"ID不能为空");
        }
        
        CheliangEntity oldCheliang = cheliangService.selectById(cheliang.getId());
        if(oldCheliang == null){
            return R.error(511,"查不到数据");
        }
        
        // 保留原有的商家ID
        if(cheliang.getShangjiaId() == null){
            cheliang.setShangjiaId(oldCheliang.getShangjiaId());
        }
    ...
    */
    ...
    public R add(@RequestBody CheliangEntity cheliang, HttpServletRequest request){
        ...
        Wrapper<CheliangEntity> queryWrapper = new EntityWrapper<CheliangEntity>()
            ...
            ;
        if(cheliang.getShangjiaId() != null){
            queryWrapper.eq("shangjia_id", cheliang.getShangjiaId());
        }
        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        CheliangEntity cheliangEntity = cheliangService.selectOne(queryWrapper);
        if(cheliangEntity==null){
            cheliang.setCheliangDelete(1);
            cheliang.setCreateTime(new Date());
            if(cheliang.getShangxiaTypes() == null){
                cheliang.setShangxiaTypes(1);
            }
            if(cheliang.getCheliangTypes() == null){
                cheliang.setCheliangTypes(1);
            }
            if(cheliang.getCheliangClicknum() == null){
                cheliang.setCheliangClicknum(0);
            }
            if(cheliang.getZanNumber() == null){
                cheliang.setZanNumber(0);
            }
            if(cheliang.getCaiNumber() == null){
                cheliang.setCaiNumber(0);
            }
            cheliangService.insert(cheliang);
            return R.ok();
        }else {
