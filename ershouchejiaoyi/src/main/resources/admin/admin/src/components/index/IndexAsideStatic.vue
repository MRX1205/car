<template>
  <el-aside class="index-aside" height="100vh" width="180px">
    <div class="index-aside-inner menulist" style="height:100%">
      <div v-for="item in menuList" :key="item.roleName" v-if="role==item.roleName" class="menulist-item" style="height:100%;broder:0;background-color:#5C92F7">
        <div class="menulistImg" style="backgroundColor:#5C92F7;padding:25px 0" v-if="false && menulistStyle == 'vertical'">
        ...
        <el-menu mode="vertical" :unique-opened="true" class="el-menu-demo" style="height:100%;" background-color="#5C92F7" text-color="#FFFFFF" active-text-color="#F2DD3D" default-active="0">
      ...
      this.$nextTick(()=>{
        document.querySelectorAll('.menulist .el-menu-item').forEach(el=>{
          el.addEventListener("mouseenter", e => {
            e.stopPropagation()
            el.style.backgroundColor = "rgba(133, 181, 255, 1)"
          })
          el.addEventListener("mouseleave", e => {
            e.stopPropagation()
            el.style.backgroundColor = "#5C92F7"
          })
